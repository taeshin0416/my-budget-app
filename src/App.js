  ...
  return (
    <div style={{ fontFamily: S.font, padding: 28, maxWidth: 1100, margin: "0 auto", background: S.bg, minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <button onClick={toggleRangeMode} style={{
          padding: "7px 16px", borderRadius: 20, border: `1px solid ${S.border}`, cursor: "pointer",
          background: rangeMode ? S.text : S.bg,
          ...
          {rangeMode ? (
            <div>
              <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
                {[["list", "Transactions"], ["chart", "Chart"]].map(([tab, label]) => (
                  <button key={tab} onClick={() => setRangeTab(tab)} style={{
                    padding: "8px 18px", borderRadius: 20, border: `1px solid ${S.border}`, cursor: "pointer",
                    background: rangeTab === tab ? S.text : S.bg,
                    color: rangeTab === tab ? S.bg : S.muted,
                    fontFamily: S.font, fontWeight: 300, fontSize: 13,
                  }}>{label}</button>
                ))}
              </div>
